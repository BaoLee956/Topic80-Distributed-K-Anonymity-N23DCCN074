...
def _build_mock_data(port: int) -> List[HealthRecord]:
    ...
    all_records = [HealthRecord(**item) for item in raw_data]

    # 2. THUẬT TOÁN CHIA MẢNH DỮ LIỆU (SHARDING)
    # Dùng (port % 3) để định danh Node: VD port 8001 -> 1, 8002 -> 2, 8003 -> 0
    node_id = port % 3
    node_records = []

    # Chia bài xoay vòng: Đảm bảo không có record nào bị trùng giữa các Node
